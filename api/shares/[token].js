import { readWorkspace, sendJson, supabaseRequest } from "../_workspaceStorage.js";
...
export default async function handler(request, response) {
  try {
    ...
    const workspace = await readWorkspace(share.owner_email);
    const tree = workspace?.documents?.[share.file_id];
    ...
    return sendJson(response, 200, { token, title: file.title || "无标题", tree });
  } catch (error) {
