...
export function renderMindMapNode(topic: string, obj: NodeObj) {
  ...
  if (metadata.type === "todo") {
    return `<span class="mindmap-todo ${metadata.checked ? "is-checked" : ""}"><span class="mindmap-todo-checkbox" data-node-id="${escapeHtml(obj.id)}">${metadata.checked ? "✓" : ""}</span><span class="mindmap-todo-content">${metadata.richTextHtml ?? escapeHtml(topic)}</span></span>`;
  }
  if (metadata.type === "quote") {
    return `<span class="mindmap-quote">${metadata.richTextHtml ?? escapeHtml(topic)}</span>`;
  }
